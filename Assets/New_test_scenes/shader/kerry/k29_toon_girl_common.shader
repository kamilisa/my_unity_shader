    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                //直射光镜面反射
                half3 NdotH = dot(normal_Dir,halfPhone_Dir);
                half spec_term = max(0.0001,pow(NdotH,_SpecShininess * spec_smoothness)) * ao_Color;
                half3 spec_layer = spec_term * _SpecColor * _SpecIntensity * spec_mask;

                //边缘光
                half fresnel = 1.0 - dot(normal_Dir,view_Dir);
